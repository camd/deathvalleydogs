...
from deathvalleydogs.models import Dog
from django.template import RequestContext
from datetime import date

def index(request):
    dog_list = Dog.objects.filter(active=True).order_by('name')
    return render_to_response('dog_index.html', {'dog_list': dog_list},
                              context_instance=RequestContext(request))

def detail(request, dog_id):
    d = get_object_or_404(Dog, pk=dog_id)
    return render_to_response('dog_detail.html', {'dog': d, 'age': age(d.birth_date)},
                              context_instance=RequestContext(request))
